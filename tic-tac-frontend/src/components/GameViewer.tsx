...
import { GAME_CONSTANTS, GAME_MODE, GAME_STATUS, UI_CONFIG } from '@/config/constants';
...
export function GameViewer({ gameId }: GameViewerProps) {
  ...
  const getCellColor = (value: number) => {
    ...
    if (value === GAME_CONSTANTS.MARK_O) return 'text-gray-600';
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">You are watching this game live</p>
        </div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-black flex items-center gap-2">
                {gameData.mode === GAME_MODE.COMPETITIVE ? <Trophy className="w-6 h-6" /> : <Users className="w-6 h-6" />}
                {gameData.mode === GAME_MODE.COMPETITIVE ? 'Competitive Game' : 'Friendly Game'}
              </h2>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className={`p-3 rounded-lg border ${
                gameData.turn % 2 === 0 && !gameOver ? 'bg-black text-white border-black' : 'bg-gray-50 border-gray-200'
              }`}>
                <div className={`text-sm ${gameData.turn % 2 === 0 && !gameOver ? 'text-gray-300' : 'text-gray-600'}`}>
                  Player X
                </div>
                <div className={`font-mono text-sm ${gameData.turn % 2 === 0 && !gameOver ? 'text-white' : 'text-black'}`}>
                  {truncateAddress(gameData.x)}
                </div>
              </div>
              <div className={`p-3 rounded-lg border ${
                gameData.turn % 2 === 1 && !gameOver ? 'bg-black text-white border-black' : 'bg-gray-50 border-gray-200'
              }`}>
                <div className={`text-sm ${gameData.turn % 2 === 1 && !gameOver ? 'text-gray-300' : 'text-gray-600'}`}>
                  Player O
                </div>
                <div className={`font-mono text-sm ${gameData.turn % 2 === 1 && !gameOver ? 'text-white' : 'text-black'}`}>
            ...
            {/* Prize Pool */}
            {gameData.mode === GAME_MODE.COMPETITIVE && gameData.stakeAmount > 0 && (
              <div className="mb-4 p-3 rounded-lg bg-yellow-50 border border-yellow-200">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-yellow-800">Prize Pool</span>
                  <span className="text-lg font-bold text-yellow-900">
                    {((gameData.stakeAmount * 2) / 1_000_000_000).toFixed(2)} SUI
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="text-center p-3 rounded-lg bg-gray-50 border border-gray-200">
              {gameOver ? (
                winner ? (
                  ...
                  <span className="text-gray-600 font-medium">Game Draw!</span>
                )
              ) : (
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <p>You are viewing this game as a spectator. The board updates automatically every 5 seconds.</p>
              </div>
            </div>
          </div>
        </div>
